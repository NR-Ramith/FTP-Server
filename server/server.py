...
def user_authentication():
    ...
    conn.send(b"1")
    passwd_size = struct.unpack("h", conn.recv(2))[0]
    passwd = conn.recv(passwd_size)
    # Send message to let client know password received
    conn.send(b"1")
    #Check if user name exsits
    num=0
    if((user_name==(b"RAMITH") and passwd==(b"RAM")) or (user_name==(b"PRANAV") and passwd==(b"PRA")) or (user_name==(b"KISHAN") and passwd==(b"KIS"))):
        num=1
